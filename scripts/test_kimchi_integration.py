...
from src.analysis.kimchi_premium import KimchiPremiumData, PremiumSignal, kimchi_calculator
...
class IntegrationTestSuite:
        ...
        try:
            ...
            for upbit_price, binance_price in test_prices:
                data = await kimchi_calculator.calculate_premium(
                    upbit_price, binance_price, exchange_rate
                )

                print(
                    f"[PASS] Premium calculated: {data.premium_rate:.2f}% "
    ...
    async def test_trading_opportunities(self):
        ...
        try:
            ...
            # 거래 기회 찾기
            opportunities = premium_storage.get_trading_opportunities(threshold=5.0, hours=1)
    ...
    async def test_data_persistence(self):
        ...
        try:
            # 히스토리 로드
            df = premium_storage.load_history(start_date=datetime.now() - timedelta(hours=1))
            ...
            if df.empty:
                ...
                df_resampled = premium_storage.export_to_dataframe(
                    start_date=datetime.now() - timedelta(hours=1), resample="5T"
                )
