...
class WebSocketManager:
    ...
    def __init__(self):
        """초기화"""
        self.exchanges = {}
        self.callbacks = {"ticker": [], "orderbook": [], "trades": [], "ohlcv": []}
        ...
        # 연결 상태
        self.connection_status = {"upbit": False, "binance": False}

        # 하트비트
        self.last_heartbeat = {"upbit": None, "binance": None}
    ...
    async def initialize_exchanges(self):
        ...
        try:
            ...
            self.exchanges["binance"] = ccxt_pro.binance(
                {
                    "enableRateLimit": True,
                    "options": {"defaultType": "future", "watchOrderBook": {"depth": 20}},  # 선물
                }
            )
        ...
        while self.is_running:
            try:
                ...
                metrics_collector.record_api_latency(
                    exchange_id, "ticker", ticker.get("info", {}).get("latency", 0)
                )
    ...
    async def watch_orderbook(self, exchange_id: str, symbol: str, limit: int = 20):
        ...
        while self.is_running:
            try:
                ...
                metrics_collector.update_balance(exchange_id, "liquidity", liquidity_score)
    ...
    async def start(self, symbols: Dict[str, List[str]]):
        ...
        for exchange_id, symbol_list in symbols.items():
            for symbol in symbol_list:
                # 티커 구독
                tasks.append(asyncio.create_task(self.watch_ticker(exchange_id, symbol)))

                # 오더북 구독
                tasks.append(asyncio.create_task(self.watch_orderbook(exchange_id, symbol)))

                # OHLCV 구독
                tasks.append(asyncio.create_task(self.watch_ohlcv(exchange_id, symbol)))
            ...
            for exchange_id in self.connection_status:
                if self.last_heartbeat.get(exchange_id):
                    ...
                    if time_since_heartbeat > 60:  # 60초 이상 응답 없음
                        logger.warning(
                            f"No heartbeat from {exchange_id} for {time_since_heartbeat:.0f}s"
                        )
                        self.connection_status[exchange_id] = False
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        try:
            # 10초 동안 실행
            await asyncio.wait_for(ws_manager.start(symbols), timeout=10)
