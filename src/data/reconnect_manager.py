...
class ReconnectManager:
    ...
    async def reconnect(self, connection_id: str, connect_func: Callable) -> bool:
        ...
        while self.attempt_counts[connection_id] < self.max_attempts:
            ...
            logger.info(f"Reconnection attempt {attempt}/{self.max_attempts} for {connection_id} "
                       f"(delay: {delay:.1f}s)")
    ...
    def get_connection_stats(self, connection_id: str) -> Dict[str, Any]:
        ...
        uptime = None
        if self.last_connect_times[connection_id] and \
           self.states[connection_id] == ConnectionState.CONNECTED:
    ...
    def get_all_stats(self) -> Dict[str, Any]:
        ...
        stats['summary'] = {
            ...
            'disconnected': sum(1 for s in self.states.values() if s == ConnectionState.DISCONNECTED),
            'reconnecting': sum(1 for s in self.states.values() if s == ConnectionState.RECONNECTING),
    ...
    async def _monitor_loop(self):
        ...
        while self.is_running:
            try:
                ...
                # 실패한 연결 체크
                failed_connections = [
                    cid for cid, stat in stats.items()
