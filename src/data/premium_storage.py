...
class PremiumStorage:
    ...
    def get_trading_opportunities(
        self, threshold: float = 4.0, hours: int = 24
    ) -> List[Dict[str, Any]]:
        """
        거래 기회 찾기
        ...
        # 임계값 이상인 경우 필터링
        opportunities = df[(df["premium_rate"] > threshold) | (df["premium_rate"] < -threshold)]
        ...
        for signal in df["signal"].unique():
            ...
            if len(signal_df) > 0:
                ...
                performance[signal] = {
                    ...
                    "win_rate": (
                        (signal_df["premium_change"] > 0).mean() if len(signal_df) > 1 else None
                    ),
                }
