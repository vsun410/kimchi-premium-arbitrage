...
class KimchiPremiumCalculator:
    ...
    async def calculate_premium(
        self, upbit_price: float, binance_price: float, exchange_rate: Optional[float] = None
    ) -> KimchiPremiumData:
        """
        김치 프리미엄 계산
        ...
        try:
            ...
            confidence = self._calculate_confidence(
                premium_rate, liquidity_score, spread_upbit, spread_binance
            )
    ...
    def get_volatility(self, hours: int = 24) -> Optional[float]:
        ...
        recent_data = [d.premium_rate for d in self.premium_history if d.timestamp > cutoff]
    ...
    async def start_monitoring(self, interval: int = 10):
        ...
        async def on_ticker_update(data):
            ...
            if hasattr(self, "_last_upbit_price") and hasattr(self, "_last_binance_price"):
                ...
                # 김프 계산
                await self.calculate_premium(self._last_upbit_price, self._last_binance_price)
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        # 김프 계산
        data = await kimchi_calculator.calculate_premium(upbit_price, binance_price, exchange_rate)
