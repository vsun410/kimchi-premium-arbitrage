...
class AlertManager:
    ...
    async def _send_slack(self, title: str, message: str, 
                         level: AlertLevel, details: Optional[Dict] = None):
        """Slack 알림 전송"""
        try:
            # 색상 설정
            color_map = {
    ...
    async def send_trade_alert(self, action: str, exchange: str, symbol: str,
...
def send_trade_alert_sync(action: str, exchange: str, symbol: str,
                          amount: float, price: float, **kwargs):
    """동기 거래 알림 전송"""
    asyncio.create_task(
