...
class MetricsCollector:
    ...
    def _setup_prometheus_metrics(self):
        ...
        # 히스토그램
        self.latency_histogram = Histogram('api_latency_seconds', 'API latency',
                                          ['exchange', 'operation'])
        self.trade_size_histogram = Histogram('trade_size', 'Trade size distribution',
                                             ['symbol'])
    ...
    def record_api_latency(self, exchange: str, operation: str, latency_ms: float):
        ...
        if PROMETHEUS_AVAILABLE and hasattr(self, 'latency_histogram'):
            self.latency_histogram.labels(
                exchange=exchange,
                operation=operation
            ).observe(latency_ms / 1000)  # 초 단위로 변환
        
        logger.debug(f"API latency recorded: {exchange} {operation} = {latency_ms:.2f}ms")
    
    def record_trade(self, exchange: str, symbol: str, side: str, 
                    amount: float, price: float, success: bool = True):
        """거래 기록"""
    ...
    def record_error(self, error_type: str, exchange: Optional[str] = None, 
